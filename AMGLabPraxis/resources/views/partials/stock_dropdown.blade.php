    ...
    if (!isset($global_giacenza_count) || !isset($global_recent_alerts)) {
        try {
            ...
            $global_recent_alerts = \App\Models\Practice::where('stato','in_giacenza')
                ->orderBy('data_arrivo','asc')->limit(8)->get(['id','codice','cliente_nome','data_arrivo']);
        } catch (\Exception $e) {
...
<div class="nav-item dropdown dropdown-hover">
    ...
    <ul class="dropdown-menu dropdown-menu-end shadow-sm p-2" aria-labelledby="giacenzaDropdown"
        ...
        <li class="px-2">
            <strong class="d-block">Pratiche in giacenza ({{ $global_giacenza_count ?? 0 }})</strong>
        </li>
        <li><hr class="dropdown-divider"></li>

        @if(($global_recent_alerts ?? collect())->isEmpty())
            <li><span class="dropdown-item-text text-muted">Nessuna pratica in giacenza</span></li>
        @else
            <div style="max-height:300px; overflow:auto;">
                ...
                <li>
                    ...
                    <div>
                        <strong>{{ $a->codice }}</strong><br>
                        <small class="text-muted">{{ $a->cliente_nome }}</small>
                    </div>
                    <small class="text-muted">{{ \Carbon\Carbon::parse($a->data_arrivo)->format('d/m/Y') }}</small>
                    </a>
                </li>
                @endforeach
            </div>
